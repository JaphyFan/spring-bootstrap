...
pipeline {
    ...
    environment {
        ORG_NAME = "japhy"
        APP_NAME = "springboot-redis"
        APP_VERSION = "1.0-SNAPSHOT"
        APP_CONTEXT_ROOT = "/"
        APP_LISTENING_PORT = "9000"
    ...
    stages {
        stage('Compile') {
            steps {
                ...
                sh "./mvnw clean compile -pl springboot-redis"
            }
        }
        ...
        stage('Unit tests') {
            steps {
                ...
                sh "./mvnw test -pl springboot-redis"
...
//        }
        ...
        stage('Package') {
            steps {
                ...
                sh "./mvnw package -DskipTests -pl springboot-redis"
        ...
        stage('Integration tests') {
            steps {
                ...
                sh "./mvnw -pl springboot-redis failsafe:integration-test failsafe:verify -DargLine=\"-Dtest.selenium.hub.url=http://selenium-hub:4444/wd/hub -Dtest.target.server.url=http://${TEST_CONTAINER_NAME}:${APP_LISTENING_PORT}/${APP_CONTEXT_ROOT}\""
        ...
        stage('Performance tests') {
            steps {
                ...
                sh "./mvnw -pl springboot-redis jmeter:configure jmeter:jmeter jmeter:results -Djmeter.target.host=${TEST_CONTAINER_NAME} -Djmeter.target.port=${APP_LISTENING_PORT} -Djmeter.target.root=${APP_CONTEXT_ROOT}"
